...
class Klient:
    ...
    def make_client(self):
        ...
        return Master(_client)  # wywolanie klasy Master w API
...
class Odczyt:
    ...
    def _read_boll(self):
        coil = self.client.read_bool(self.unit, self._reg_start, self._reg_lenght, self.reg_type)
        return coil
...
if __name__ == '__main__':
    ...
    reg = Odczyt(client, unit=3, reg_start=0, reg_lenght=1, data_type='int')
    # bool = Odczyt(client,unit=3,reg_start=0,reg_lenght=250,)

    try:
        while True:
            print(nr, end=': ')
            for i in reg_for_check:
                reg.set_reg_adress(i)
